...
import Fechamentotelefonicaedicao from '../fechamento/Fechamentotelefonicaedicao';


const Telefonicafechamento = ({ setshow, show }) => {
  ...
  const lista = async () => {
    ...
    try {
      setloading(true);
      await api.get('v1/projetozte/fechamento', { params }).then((response) => {
  ...
  function alterarUser(descempresa, descidempresa) {
    settelacadastroedicao(true);
  ...
  const columns = [
    {
      ...
      getActions: (parametros) => [
        <GridActionsCellItem
          icon={<EditIcon />}
          label="Alterar"
          onClick={() => alterarUser(parametros.row.nome, parametros.row.idempresa)}
        />,
      ],
    },
  ...
  return (
    ...
    >
      <ModalHeader toggle={togglecadastro.bind(null)} style={{ backgroundColor: "white" }}>Fechamento Telefonica</ModalHeader>
      <ModalBody style={{ backgroundColor: "white", padding: "0px" }}>

        <div>
          <Card>
            <CardBody className = "mb-0" style={{ backgroundColor: 'white' }}>
              {mensagem.length !== 0 ? (
...
Telefonicafechamento.propTypes = {
  show: PropTypes.bool.isRequired,
  setshow: PropTypes.func.isRequired,
};

export default Telefonicafechamento;
