...
import Extratofechamentocosmx from './Extratofechamentocosmx';
...
const Fechamentocosmxedicao = ({ setshow, show, idempresa, empresa, email }) => {
  ...
  const [datansemana, setdatansemana] = useState();
  ...
  const lista = async () => {
    try {
      setloading(true);
      const response = await api.get('v1/projetocosmxid/fechamentoporempresa', { params });
      const { data } = response;
      ...
      setprojeto(validatedData);
      setmensagem('');
      console.log('Projetos: ', validatedData);
    } catch (err) {
  ...
  const listahistorico = async () => {
    try {
      setloading(true);
      const response = await api.get('v1/projetocosmxid/historicopagamento', { params });
      const { data } = response;
  ...
  const historicopagamento = [
    ...
    /*{
      field: 'inicioatividadereal',
      headerName: 'INICIO ATIVIDADE REAL',
      width: 170,
      align: 'left',
      editable: false,
    }, */
    {
      field: 'aprovacaocosmx',
      headerName: 'APROVAÇÃO COSMX',
      width: 150,
      align: 'left',
      editable: false,
    },
        ...
        <GridActionsCellItem
          ...
          onClick={() =>
            selecao(
              ...
              parametros.row.sitefromto,
              parametros.row.valorlpu,
  ...
  function extratopagamento() {
    if (!datansemana) {
      setmensagem('Informe o ANO/SEM de pagamento');
      return;
    }
    settelaextrato(true);
  }
  ...
  function salvapagamento(e) {
    ...
    setmensagemsucesso('');
    api
      .post('v1/projetocosmxid/fechamento/salvapagamento', {
          ...
          <FormGroup>
            <div className="row g-3">
              ...
              <div className="col-sm-1">
                ...
                <Input
                  type="text"
                  onChange={(e) => setdatansemana(e.target.value)}
                  value={datansemana}
                />
              </div>
...
Fechamentocosmxedicao.propTypes = {
...
export default Fechamentocosmxedicao;
