...
const Extratofechamentocosmx = ({
  ...
  const [emailadcional, setemailadcional] = useState(
    'anna.christina@telequipeprojetos.com.br  ; ingrid.santos@telequipeprojetos.com.br ; juliana.yamaguchi@telequipeprojetos.com.br ; financeiro@telequipeprojetos.com.br ',
  );
  ...
  const lista = async () => {
    try {
      setloading(true);
      await api.get('v1/projetocosmxid/extrato', { params }).then((response) => {
  ...
  /*  const listadesconto = async (valort) => {
        try {
          setloading(true);
          await api.get('v1/projetocosmxid/extratodesconto', { params }).then((response) => {
  ...
  const listatotal = async () => {
    try {
      setloading(true);
      await api.get('v1/projetocosmxid/extratototal', { params }).then((response) => {
  ...
  const apagarpagamento = () => {
    setmensagem('');
    api
      .post('v1/projetocosmxid/fechamento/apagapagamento', {
        id,
      })
      .then((response) => {
  ...
  const columns = [
    ...
    {
      field: 'nomerelatorioenviado1',
      headerName: 'RELATÓRIO ACEITAÇÃO',
      width: 280,
      align: 'left',
      editable: false,
    },
  ...
  const enviaremail = () => {
    ...
    console.log(emailpj);
    api
      .post('v1/email/acionamentopj/extratocosmx', {
...
Extratofechamentocosmx.propTypes = {
...
export default Extratofechamentocosmx;
