...
const Ericssoncontrolelpu = ({ setshow, show }) => {
  ...
  const listalpu = async () => {
    try {
      setloading(true);
      await api.get('v1/projetoericsson/selectlpu', { params }).then((response) => {
  ...
  const lista = async () => {
    ...
    try {
      setloading(true);
      await api.get('v1/projetoericsson/listagemlpu', { params }).then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: 'Descricão Atividade',
      width: 300,
      align: 'left',
      editable: false,
    },
  ...
  const uploadarquivo = async (e) => {
    ...
    formData.append('files', arquivolpu);
    ...
    try {
      setloading(true);
      const response = await api.post('v1/uploadlpu', formData, header);
...
Ericssoncontrolelpu.propTypes = {
  show: PropTypes.bool.isRequired,
  setshow: PropTypes.func.isRequired,
};

export default Ericssoncontrolelpu;
