...
const AdicionarSiteManual = ({ show, setShow, onSiteAdded, empresa = 'telefonica' }) => {
  ...
  const checkDuplicate = async () => {
    try {
      ...
      const response = await api.get(`v1/rollout${empresa}/verificarduplicidade`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(`v1/rollout${empresa}/adicionarmanual`, payload);
...
AdicionarSiteManual.defaultProps = {
  empresa: 'telefonica',
};
