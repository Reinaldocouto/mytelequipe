...
const Rollouttelefonicaedicao = ({
  ...
  const [enggabinetenovo, setEngGabineteNovo] = useState('');
  const [enggradilgabinete, setEngGradilGabinete] = useState('');
  const [engdetentoragabinete, setEngDetentoraGabinete] = useState('');
  const [mastericn, setmastericn] = useState('');
  const [pmotecnequip, setpmotecnequip] = useState('');
  ...
  const [pmoaceitacaoplan, setpmoaceitacaoplan] = useState('');
  const [pmoaceitacaoreal, setpmoaceitacaoreal] = useState('');
  ...
  useEffect(() => {
    if (uididpmts) {
      setTpForm((prev) => ({ ...prev, siteId: uididpmts }));
    }
  }, [uididpmts, pmuf, idr]);
  ...
  const listaid = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setEngGabineteNovo(response.data?.enggabinetenovo);
        setEngGradilGabinete(response.data?.enggradilgabinete);
        setEngDetentoraGabinete(response.data?.engdetentoragabinete);
        setmastericn(response.data?.mastericn);
        setpmotecnequip(response.data?.pmotecnequip);
        ...
        const acePlan = response.data.pmoaceitacaop ?? response.data.pmo_aceitacao_p ?? '';
        const aceReal = response.data.pmoaceitacaor ?? response.data.pmo_aceitacao_r ?? '';
        setdocaplan(trataData(response.data.docaplan));
        setpmoaceitacaoplan(trataData(acePlan));
        setpmoaceitacaoreal(trataData(aceReal));
  ...
  const listapessoasresponsavel = async () => {
    try {
      const response = await api.get('/v1/pessoa', {
        params: {
          mostrarInativos: 1,
          regional: pmuf,
        },
  ...
  const handleResponsibleTeamChange = (selectedOptions) => {
    ...
    // Filtra e bloqueia opções inativas
    const ativos = arr.filter((item) => !item.label?.includes('(INATIVO)'));

    // Se havia inativos, avisa o usuário
    if (arr.length !== ativos.length) {
      toast.warning('Pessoas inativas não podem ser selecionadas.');
    }

    // Atualiza somente com os ativos
    setEquipeResponsavel(ativos);
  };
  ...
  const colunasatividades = [
    ...
    },
    {
      ...
      editable: true,
      type: 'singleSelect',
      valueOptions: ['GREENFIELD', 'BROWNFIELD'],
    },
    {
    ...
    },
    {
      ...
      type: 'singleSelect',
      editable: true,
      valueOptions: ['AGUARDANDO', 'APROVADO', 'REPROVADO'],
    },
  ];
          ...
          <div>
            ...
            <CardBody className="px-4 , pb-2">
              <div className="row g-3">
                ...
                <div className="col-sm-2">
                  ENG_GABINETE_NOVO
                  <Input type="text" value={enggabinetenovo} disabled />
                </div>
                <div className="col-sm-2">
                  ENG_GRADIL_GABINETE
                  <Input type="text" value={enggradilgabinete} disabled />
                </div>
                <div className="col-sm-2">
                  ENG_DETENTORA_GABINETE
                  <Input type="text" value={engdetentoragabinete} disabled />
                </div>
                <div className="col-sm-2">
                  MASTERSITE_CN
                  <Input type="text" value={mastericn} disabled />
                </div>
                <div className="col-sm-2">
                  PMO_TECN_EQUIP
                  <Input type="text" value={pmotecnequip} disabled />
                </div>
                <div className="col-sm-2">
            ...
            <CardBody className="px-4 , pb-2">
              ...
              <div className="row g-3">
                ...
                <div className="col-sm-2">
                  ...
                  <Input
                    type="date"
                    onChange={(e) => setpmoaceitacaoplan(e.target.value)}
                    value={pmoaceitacaoplan}
                  />
                </div>
                <div className="col-sm-2">
                  PMO - Aceitação Real
                  <Input
                    type="date"
                    onChange={(e) => setpmoaceitacaoreal(e.target.value)}
                    value={pmoaceitacaoreal}
                  />
                </div>
              </div>
              <br />
