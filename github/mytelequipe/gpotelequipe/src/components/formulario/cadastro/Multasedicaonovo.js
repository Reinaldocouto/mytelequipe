...
import { toast, ToastContainer } from 'react-toastify'; // <-- ToastContainer adicionado aqui
...
5100\tEntregar o veículo a pessoa sem usar lentes corretoras de visão e Entregar o veículo a pessoa sem usar aparelho auxiliar de audição e Entregar o veículo a pessoa sem aparelho de prótese física e Entregar veíc pessoa s/ adaptações impostas concessão/renovação licença conduzir
...
function parseNaturezaPontuacao(line) {
  ...
  if (match) {
    const pontuacao = match[1] ? match[1].trim() : '';
    const natureza = match[2].trim();
    return { pontuacao, natureza };
  }
  return { pontuacao: '', natureza: line.trim() };
}
...
const Multasedicao = ({ setshow, show, ididentificador, atualiza }) => {
  ...
  const listamultas = async () => {
    try {
      ...
      await api.get('v1/multasid', { params }).then((response) => {
        ...
        setdatacolaborador(data.datacolaborador);
  ...
  function ProcessaCadastro(e) {
    e.preventDefault();
    //console.log("Função ProcessaCadastro iniciada");
    ...
    if (!datainfracao || (typeof datainfracao === 'string' && datainfracao.trim() === '')) {
    ...
    if (!idpessoa) {
      // console.log("Erro: Nome Indicado não selecionado! Valor de idpessoa:", idpessoa);
    ...
    const data = {
      ...
      numeroait: numeroait ? parseInt(numeroait, 10) : null,
      pontuacao: pontuacao ? parseInt(pontuacao, 10) : 0, // Evita erro se não houver pontuação
      nomeindicado,
      placa,
      datainfracao,
      local,
  ...
  return (
    <>
      ...
      >
        <ModalHeader toggle={togglecadastro.bind(null)}>Multas</ModalHeader>
        <ModalBody>
          {mensagem.length > 0 ? (
              ...
              <div className="row g-3">
                ...
                <Col md="2">
                  <FormGroup>
                    ...
                    <Input
                      type="text"
                      onChange={(e) => setnumeroait(e.target.value)}
                      value={numeroait}
                      placeholder=""
                    />
                  </FormGroup>
                ...
                <Col md="3">
                  <FormGroup>
                    Natureza
                    <Input type="text" value={natureza} placeholder="" readOnly />
                  </FormGroup>
                </Col>
                <Col md="2">
                  <FormGroup>
                ...
                <Col md="5">
                  <FormGroup>
                    ...
                    <Input
                      type="text"
                      onChange={(e) => setstatusmulta(e.target.value)}
                      value={statusmulta}
                      name=""
                    />
                  </FormGroup>
      ...
      {/* Aqui está a inclusão do ToastContainer */}
      <ToastContainer
        position="top-right"
        autoClose={5000}
