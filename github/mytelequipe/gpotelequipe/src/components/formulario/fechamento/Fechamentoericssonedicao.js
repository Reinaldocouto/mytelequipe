...
const Fechamentoericssonedicao = ({ setshow, show, empresa, email, regional }) => {
  ...
  const [status, setStatus] = useState('Antecipação');
  ...
  const [dataMespagamento, setdataMespagamento] = useState('');
  ...
  const lista = async () => {
    try {
      setloading(true);
      const response = await api.get('v1/projetoericsson/fechamentoporempresa', { params });
      const { data } = response;
      setprojeto(data);
  ...
  const listahistorico = async () => {
    try {
      setloading(true);
      const response = await api.get('v1/projetoericsson/historicopagamento', { params });
      const { data } = response;
      setprojetohistorico(data);
      //      console.log("Historico: ", projetohistorico)

      // Filtrando poitems que correspondem ao datapagamento e numerol
      const encontraPoItem = data
        .filter((item) => item.mespagamento === dataMespagamento)
  ...
  const selecao = (stat, codigo, servico, valorservico, porcentagem, observacaoint) => {
    listapagamento();
        ...
        <GridActionsCellItem
          ...
          onClick={() =>
            selecao(
              ...
              parametros.row.observacaointerna,
            )
          }
        />,
      ],
            ...
            <div className="row g-3">
              ...
              <div className="col-sm-2">
                ...
                <Input
                  type="month"
                  onChange={(e) => {
                    console.log(dataMespagamento);
                    setdataMespagamento(e.target.value);
                  }}
                  value={dataMespagamento}
              ...
              {/* <div className="col-sm-1">
                ...
                <Input
                  type="select"
                  onChange={(e) => setnumerol(parseInt(e.target.value, 10))} //transforma em numero
                  value={numerol}
                >
