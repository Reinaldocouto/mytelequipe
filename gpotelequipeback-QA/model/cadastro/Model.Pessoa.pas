...
begin
  try
    ...
    if AQuery.ContainsKey('mostrarInativos') then
      mostrarInativos := SameText(AQuery.Items['mostrarInativos'], '1');
    ...
    begin
      ...
      // Se mostrarInativos estiver habilitado, adiciona "(INATIVO)" no nome
      if mostrarInativos then
      begin
        SQL.Add('  CASE ');
        SQL.Add('    WHEN gespessoa.deletado = 1 THEN CONCAT(gespessoa.nome, " (INATIVO)") ');
        SQL.Add('    ELSE gespessoa.nome ');
        SQL.Add('  END AS nome, ');
      end
      else
      begin
        SQL.Add('  gespessoa.nome AS nome, ');
      end;

      // Campo auxiliar booleano
      SQL.Add('  (gespessoa.deletado = 0) AS ativo, ');

      // Demais campos
      SQL.Add('  gespessoa.*, ');
      SQL.Add('  gesempresas.regional AS regionalEmpresa ');

      SQL.Add('FROM gespessoa ');
      SQL.Add('LEFT JOIN gesempresas ON gespessoa.empresa = gesempresas.idempresa ');
      SQL.Add('WHERE gespessoa.idpessoa IS NOT NULL ');

      // 🔍 Filtro de busca
      if AQuery.ContainsKey('busca') then
        if Length(AQuery.Items['busca']) > 0 then
          SQL.Add('AND (gespessoa.nome LIKE :busca)');
      ...
      begin
        SQL.Add('AND gespessoa.regional = :regional');
        SQL.Add('AND gesempresas.regional = :regional');
        ParamByName('regional').Value := AQuery.Items['regional']; // ← sem .ToInteger
      end;

      // ✅ Mostra apenas ativos, exceto se mostrarInativos estiver habilitado
      if not mostrarInativos then
        SQL.Add('AND gespessoa.deletado = 0');

      SQL.Add('ORDER BY nome');

      // Aplicar parâmetro busca se existir
      if AQuery.ContainsKey('busca') then
        if Length(AQuery.Items['busca']) > 0 then
          ParamByName('busca').Value := '%' + AQuery.Items['busca'] + '%';
