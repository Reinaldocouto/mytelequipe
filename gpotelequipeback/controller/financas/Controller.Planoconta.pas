unit Controller.Planoconta;

interface

uses
  Horse, System.JSON, System.SysUtils, FireDAC.Comp.Client, Data.DB,
  DataSet.Serialize, Model.Planoconta, UtFuncao, Controller.Auth;

procedure Registry;

procedure Listacredito(Req: THorseRequest; Res: THorseResponse; Next: TProc);
procedure Listadebito(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
procedure Registry;
begin
  THorse.get('v1/planocontacredito', Listacredito);
  THorse.get('v1/planocontadebito', Listadebito);
  THorse.get('v1/planoconta', Lista);
  THorse.get('v1/planocontaid', Listaid);
  THorse.post('v1/planoconta', salva);
  THorse.Post('v1/planoconta/novocadastro', novocadastro);
end;
...
begin
  ...
  try
   ...
   {   if servico.NovoCadastro(erro) > 0 then
        Res.Send<TJSONObject>(CreateJsonObj('retorno', servico.idplanoconta)).Status(THTTPStatus.Created)
      else
...
begin
  ...
  qry := servico.Listacredito(Req.Query.Dictionary, erro);
  try
    try
...
begin
  ...
  qry := servico.Listadebito(Req.Query.Dictionary, erro);
  try
    try
      arraydados := qry.ToJSONArray();
      if erro = '' then
...
begin
  ...
  try
    try
      body := Req.body<TJSONObject>;
      if strisint(body.getvalue<string>('idplanoconta', '')) then
        servico.idplanoconta := body.getvalue<integer>('idplanoconta', 0)
      else
        servico.idplanoconta := 0;
      ...
      servico.naocontabilizado := body.getvalue<string>('naocontabilizado','');
