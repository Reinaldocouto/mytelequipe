...
uses
  ...
  Controller.Auth, System.Zip, System.IOUtils, System.Classes,  System.Threading;
...
procedure ProcessarArquivoObra(const vDiretorio: string);
const
    ...
    LUploadConfig.UploadFileCallBack :=
      ...
      begin
        ...
        TTask.Run(
          ...
          begin
            try
              ProcessarArquivoObra(vDiretorio);
            except
              on E: Exception do
                Writeln('Erro no processamento: ' + E.Message);
            end;
          end
        );
      end;

    // Callback quando todos os arquivos terminarem de chegar
    LUploadConfig.UploadsFishCallBack :=
      procedure(Sender: TObject; AFiles: TUploadFiles)
      begin
        Res.Send<TJSONObject>(
          CreateJsonObj('sucesso', 'Upload concluído. Processamento em segundo plano.')
    ...
    try
      ...
      LUploadConfig.UploadFileCallBack :=
        ...
        begin
          ...
          try
            Writeln('[' + TimeToStr(Now) + '] Iniciando processamento do arquivo: ' + AFile.FileName + ' (' + AFile.Size.ToString + ' bytes)');
            ...
            try
                ...
                try
                  ...
                  begin
                    try
                      ...
                      Writeln(Format('[%s] Lote %d (%d-%d) processado com sucesso. ' + 'Registros: %d/%d. Tempo: %s', [TimeToStr(Now), (indice div TAMANHO_LOTE) + 1, indice + 1, fimLote + 1, resultado, tamanhoLoteAtual, FormatDateTime('nn:ss', Now - inicioLote)]));

                    except
                      on E: Exception do
                      begin
                        erro := Format('Tentativa %d/%d - Erro no lote %d: %s', [tentativa, MAX_TENTATIVAS, (indice div TAMANHO_LOTE) + 1, E.Message]);
                        ...
                        begin
                          ...
                          Writeln(Format('[%s] ERRO: Falha ao processar lote %d após %d tentativas', [TimeToStr(Now), (indice div TAMANHO_LOTE) + 1, MAX_TENTATIVAS]));
                        end;
              ...
              Writeln(Format('[%s] PROCESSAMENTO CONCLUÍDO! ' + 'Total: %d registros | Sucesso: %d (%.2f%%) | Erros: %d | Lotes com erro: %d', [TimeToStr(Now), jsonData.Count, totalProcessados, taxaSucesso, totalErros, lotesComErro]));
...
begin
  ...
  try
    ...
    begin
      Res.Send<TJSONObject>(TJSONObject.Create.AddPair('erro', 'Erro ao criar diretório de upload')).Status(THTTPStatus.InternalServerError);
      Exit;
    end;
              ...
              try
                ...
                begin
                 { linhasJSON.Add(TJSONObject.Create
                  ...
                  linhasJSON.Add(TJSONObject.Create
                  .AddPair('linha', TJSONNumber.Create(i + 1))
                  .AddPair('status', 'sucesso')
                  .AddPair('id', TJSONString.Create(linhaJSON.GetValue<string>('id'))));
    ...
    LUploadConfig.UploadsFishCallBack :=
      ...
      begin
        ...
        try
          resposta.AddPair('sucesso', TJSONBool.Create(True));
          resposta.AddPair('resultados', resultados); // se 'resultados' já for TJSONArray ou TJSONObject
          System.Writeln('Finalizado - Importação do T2');
    ...
    begin
      ...
      Res.Send<TJSONObject>(TJSONObject.Create.AddPair('erro', 'Erro no upload').AddPair('detalhes', E.Message)).Status(THTTPStatus.InternalServerError);
    end;
  end;
end;
        ...
        try
          ...
          try
            ...
            begin
              linhaJSON := jsonData.Items[i] as TJSONObject;
              try
                if servico.EditarT4(linhaJSON, periodo, erroLinha) > 0 then
                begin
                  linhasJSON.Add(
                    TJSONObject.Create
                      .AddPair('linha', TJSONNumber.Create(i + 1))
...
procedure UploadZteLpu(Req: THorseRequest; Res: THorseResponse; Next: TProc);
const
  DIRETORIO_UPLOAD = 'C:\servidorgpo\ztelpu\';
var
  servico: TUpload;
  LUploadConfig: TUploadConfig;
  Inseridos: Integer;
  erro: string;
begin
  // Cria a instância do serviço ANTES de configurar os callbacks
  servico := TUpload.Create;
  try
    // Verifica e cria o diretório se necessário
    if not DirectoryExists(DIRETORIO_UPLOAD) then
    ...
    try
      ...
      LUploadConfig.UploadFileCallBack :=
        ...
        begin
          try
            ...
            jsonData := LerExcelParaJSONLpuValidar(vXLSFile);
            Writeln(jsonData.ToString);

            if (jsonData <> nil) and (jsonData.Count > 0) and (jsonData.Items[0] is TJSONObject) then
            begin
              Inseridos := servico.InserirLpuZte(jsonData, erro); // Corrigido: usando a variável local servico
              if erro <> '' then
                Writeln('Erro ao inserir: ' + erro);
            end;
          except
            on E: Exception do
              Writeln('Erro no callback de upload: ' + E.Message);
      ...
      LUploadConfig.UploadsFishCallBack :=
        ...
        begin
          Writeln('');
          Writeln('Processados ' + Inseridos.ToString + ' registros.');
          Writeln('Finish ' + AFiles.Count.ToString + ' files.');
        end;
