...
uses
  ...
  DataSet.Serialize, Model.rh, UtFuncao, Controller.Auth;

procedure Registry;

procedure Listafolhapagamento(Req: THorseRequest; Res: THorseResponse; Next: TProc);
procedure Listavaletransporte(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
procedure Listafolhaid(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
procedure Registry;
begin
  THorse.get('v1/rh/folhapagamento', Listafolhapagamento);
  THorse.get('v1/rh/folhapagamentoid', Listafolhaid);
  THorse.get('v1/rh/valetransporte', Listavaletransporte);
  THorse.get('v1/rh/convenio', Listaconvenio);
  THorse.get('v1/rh/ticket', Listaticket);
//  THorse.get('v1/rhid', Listaid);
//  THorse.post('v1/rh', salva);
//  THorse.Post('v1/rh/novocadastro', novocadastro);
...
begin
  ...
  qry := servico.Listafolhapagamento(Req.Query.Dictionary, erro);
  try
    try
      arraydados := qry.ToJSONArray();
      if erro = '' then
...
begin
  ...
  qry := servico.Listafolhaid(Req.Query.Dictionary, erro);
  try
    try
      arraydados := qry.ToJSONArray();
      if erro = '' then
...
begin
  ...
  qry := servico.Listavaletransporte(Req.Query.Dictionary, erro);
  try
    try
      arraydados := qry.ToJSONArray();
      if erro = '' then
...
begin
  ...
  qry := servico.Listaticket(Req.Query.Dictionary, erro);
  try
    try
      arraydados := qry.ToJSONArray();
      if erro = '' then
