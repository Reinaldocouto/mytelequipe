unit Controller.Subcategoria;

interface

uses
  Horse, System.JSON, System.SysUtils, FireDAC.Comp.Client, Data.DB,
  DataSet.Serialize, Model.Subcategoria, UtFuncao, Controller.Auth;
...
procedure Registry;
begin
  THorse.get('v1/subcategoria', Lista);
  THorse.get('v1/subcategoriaid', Listaid);
  THorse.post('v1/subcategoria', salva);
  THorse.Post('v1/subcategoria/novocadastro', novocadastro);
end;

procedure novocadastro(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var
  servico: TSubcategoria;
  body: TJSONValue;
  JSON: TJSONObject;
  erro: string;
begin
  servico := TSubcategoria.Create;
  try
    try
      ...
      if servico.NovoCadastro(erro) > 0 then
        Res.Send<TJSONObject>(CreateJsonObj('retorno', servico.idsubcategoria)).Status(THTTPStatus.Created)
      else
...
begin
  ...
  try
    ...
    try
      body := Req.body<TJSONObject>;
      if strisint(body.getvalue<string>('idsubcategoria', '')) then
        servico.idsubcategoria := body.getvalue<integer>('idsubcategoria', 0)
      else
        servico.idsubcategoria := 0;

      if strisint(body.getvalue<string>('categoria', '')) then
        servico.idcategoria := body.getvalue<integer>('categoria', 0)
      else
        servico.idcategoria := 0;

      servico.descricaosubcategoria := body.getvalue<string>('descricaosubcategoria','');
      servico.idcliente := body.getvalue<integer>('idcliente', 0);
      servico.idloja := body.getvalue<integer>('idloja', 0);
