...
type
  ...
  public
    ...
    function excluirtp(out erro: string): Boolean;
    function excluirobraericssoncrq(out erro: string): Boolean;
  end;
...
begin
  try
    ...
    qry.connection := FConn;
    try
      // Primeiro verifica se existe algum pagamento registrado (ANTES de iniciar a transação)
      with qry do
      begin
        Active := false;
        sql.Clear;
      ...
      // Se tem pagamento registrado, impede a exclusão
      ...
      // Se não tem pagamento, inicia transação e prossegue com a exclusão
      FConn.StartTransaction;
...
function TExclusao.excluirtp(out erro: string): Boolean;
var
  qry: TFDQuery;
begin
  try
    ...
    try
      ...
      begin
        Active := false;
        sql.Clear;
        SQL.Add('update tp set deletado = 1 where id=:id ');
        ParamByName('id').AsInteger := StrToInt(id);
        ExecSQL;
      end;
      erro := '';
...
function TExclusao.excluirobraericssoncrq(out erro: string): Boolean;
var
  qry: TFDQuery;
begin
  try
    ...
    try
      ...
      begin
        Active := false;
        sql.Clear;
        SQL.Add('update obraericssoncrq set deletado = 1 where id=:id ');
        ParamByName('id').AsInteger := StrToInt(id);
        ExecSQL;
      end;
      erro := '';
...
begin
  try
    ...
    try
      ...
      // se status 'ATIVO', n�o deleta
      if vehicleStatus = 'ATIVO' then
      begin
        erro := 'Primeiro � necess�rio desativar esse ve�culo.';
        Result := False;
        FConn.Rollback;
      end
      else
      begin
        // continua para dele��o
        qry.Active := False;
        qry.SQL.Clear;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            ParamByName('tipo').asstring := 'servi�o'+inttostr(cont);
            execsql;
          end;
          Next;
        end;
