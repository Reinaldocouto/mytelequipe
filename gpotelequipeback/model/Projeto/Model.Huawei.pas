...
begin
  ...
  try
    ...
    begin
      qry.SQL.Add('SELECT');
      qry.SQL.Add('  r.*,');
      qry.SQL.Add('  DATE_FORMAT(r.criado_em, ''%Y-%m-%d %H:%i:%s'') AS criadoEm,');
...
function THuawei.marcarComoAvulso(const uuidps: string; out erro: string): Boolean;
var
  qry: TFDQuery;
  ids: TArray<string>;
  i: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if i > 0 then whereIn := whereIn + ',';
        whereIn := whereIn + Trim(ids[i]);
      end;
      qry.SQL.Text :=
        'UPDATE rollouthuawei '+
        '   SET avulso = 1, deletado = 0, Ultima_atualizacao = NOW(), Ultima_Pessoa_Atualizacao = :usr '+
        ' WHERE idgeral IN ('+ whereIn +')';
    end
    else
    begin
      qry.SQL.Text :=
    ...
    Result := qry.RowsAffected > 0;
    if not Result then erro := 'Nenhum registro atualizado.';
  except
    on E: Exception do
    begin
      if FConn.InTransaction then FConn.Rollback;
...
begin
  ...
  try
    ...
    begin
      ...
      qry.SQL.Text :=
        'UPDATE rollouthuawei '+
        '   SET avulso = 0, Ultima_atualizacao = NOW(), Ultima_Pessoa_Atualizacao = :usr '+
        ' WHERE idgeral IN ('+ whereIn +')';
    end
    else
    begin
