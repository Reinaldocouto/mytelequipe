...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add('  pmtsvivo.ENG_GABINETE_NOVO AS enggabinetenovo,');
      SQL.Add('  pmtsvivo.ENG_GRADIL_GABINETE AS enggradilgabinete,');
      SQL.Add('  pmtsvivo.ENG_DETENTORA_GABINETE AS engdetentoragabinete,');
      SQL.Add('  pmtsvivo.MASTERSITE_CN AS mastericn,');
      SQL.Add('  pmtsvivo.PMO_TECN_EQUIP AS pmotecnequip,');
      SQL.Add('  CASE');
      SQL.Add('    WHEN rolloutvivo.acessodatafinal IS NULL THEN NULL');
      SQL.Add('    WHEN DATE(rolloutvivo.acessodatafinal) = CURDATE() THEN "Vencido"');
      SQL.Add('    WHEN DATE(rolloutvivo.acessodatafinal) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN "A vencer"');
      SQL.Add('    ELSE ""');
      SQL.Add('  END AS status_vencimento');
      SQL.Add('FROM rolloutvivo');
      SQL.Add('INNER JOIN pmtsvivo ON pmtsvivo.UID_IDPMTS = rolloutvivo.UIDIDPMTS');
      SQL.Add('WHERE 1=1');
