...
begin
  ...
  try
    ...
    Query.SQL.Clear;
    Query.SQL.Add('SELECT emaildiaria FROM gesemailconfiguracao WHERE tipo = ''diaria''');
    try
      Query.Open;
    ...
    begin
     // if not SendEmail(destinatarios[i], 'Nova Diária Registrada - ' + dadosDiaria.Numero, htmlEmail, erro) then
      if not SendEmail(destinatarios[i], 'Nova Diária Registrada - ' + dadosDiaria.Numero, htmlEmail,
        TPath.Combine(ExtractFilePath(ParamStr(0)), 'assets\\templates\\Solicitação Adiantamento.xlsx')) then
      begin
